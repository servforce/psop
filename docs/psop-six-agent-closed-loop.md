...
| 字段 | 类型 | 中文描述 |
|---|---|---|
| `id` | String(36) | RunTrace 记录 ID。 |
| `run_id` | String(36) | 所属 Run。 |
| `agent_run_id` | String(36) | 关联 AgentRun。 |
| `seq_no` | Integer | Run 内 trace 序号。 |
| `phase` | String(64) | Runtime 阶段。 |
| `event_type` | String(120) | runtime.node.started / runtime.node.completed / agent.called / tool.called 等。 |
| `trace_id` | String(64) | OpenTelemetry trace id，用于跨 OTel 和 Replay 关联。 |
| `span_id` | String(64) | OpenTelemetry span id；无有效 OTel span 时使用本地回退值。 |
| `parent_span_id` | String(64) | 父 span id。 |
| `payload` | JSON | Trace payload。 |
...
其中 `replay_provenance` 必须直接串联 `invocation_id / run_id / pskill_version_id / compile_artifact_id / compile_request_id / latest_session_token_snapshot_id`，与 `run_trace.trace_id / span_id` 共同支撑 Replay + OTel 排障闭环。Invocation / Run API 响应必须暴露派生 `compile_request_id` 以便运行入口可回跳编译证据。Compiler / job 关键 span 必须带上 `compile_request_id / pskill_definition_id / pskill_version_id / compile_artifact_id`；Runtime 关键 span 必须带上同一组 provenance 属性，以及当前输入 `session_token_id / session_token_seq`。
...
`/ws/runs/{run_id}` 广播运行现场增量事实：

- `terminal.event.appended`：新增 `run_event`，用于 Run Live transcript、Run Events 和 Replay timeline 增量更新。
- `trace.event.appended`：新增 `run_trace`，用于 Run Replay timeline、EG Node Path 和可观测排障增量更新。
- `session_token.snapshot.appended`：新增 `session_token_snapshot`，用于 Run Replay snapshot 比较和状态主权证据增量更新。
- `run.updated`：`run` 行状态、phase、latest sequence、final output 等元数据更新，用于 Run Live 状态栏和 Replay run metadata 增量更新。
- `binding.updated`：Run capability binding 变更，用于 Run Live Binding 列表和 Replay binding evidence 增量更新。
...
5. 前端 Compiler Workbench，编译请求列表和 Artifact 详情必须展示 progress 摘要，并能跳转 `pskill.compiler` AgentRun 与 EG Artifact 证据。
...
2. 实现测试模拟器。
3. 实现 `pskill.tester`。
4. 前端 Testing Center 和 Publish Gate，测试运行 review 必须能跳转 `pskill.tester` AgentRun 与 Runtime Replay 证据。
...
4. 前端 Run Live、Run Replay 增强，运行入口必须展示 `compile_request_id` 与 EG Artifact 回链，Replay 中 RunTrace、AgentEvent、ModelCall、ToolCall、ToolAuthorization 证据必须可跳转到 `pskill.runner` AgentRun 详情。
...
4. 前端 Run Evaluation Reports、Findings，报告详情必须能跳转 `pskill.evaluator` AgentRun 和 Replay evidence。
...
5. 前端 Governance Proposals、Experiments、Platform Tool Authorizations，提案详情必须能跳转 `psop.governance` AgentRun、来源 evaluation/finding/replay 和工具授权上下文。
...
4. Memory 页面。
5. Dashboard。
6. 全局指标和 OpenTelemetry；Platform Observability metrics 必须覆盖 Runtime、Agent、Evaluation、Governance 与工具授权状态。
