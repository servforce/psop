...
`previous_validation_summary` 只读取当前 generation 之前最近一条终态 generation。该记录为 `failed` 且携带 Builder validation diagnostics 时才向模型提供反馈；最近记录为 `succeeded`、`cancelled` 或非校验失败时保持为空，不得回溯更早的失败诊断。
...
| --- | --- | --- | --- | --- | --- |
| `psop.builder.read_current_source` | read_only | none | allow | 40k chars | 使用空对象参数从 invocation context 返回当前 README/SKILL、目标快照和辅助文件快照。 |
...
`psop.builder.read_current_source` 用于读取当前 Skill source，不访问 GitLab 或数据库。它使用空对象参数，从本次 invocation context 中固定返回 README/SKILL；精确基线还返回结构化目标和辅助文件快照。
