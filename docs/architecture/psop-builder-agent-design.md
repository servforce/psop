...
7. 平台校验与提交
   ...
   - 校验通过后，`submit_candidate` 物化最终 Markdown；图片类参考资产保存到 `outputs/skill-draft/references/`，并在使用该图片的流程步骤中通过相对 Markdown 图片链接展示，再由平台提交 GitLab draft。
...
`AgentInvocation.input` 还必须实际包含 `generation_intent`、`evidence_policy`、`previous_validation_summary` 和紧凑的 `revision_baseline` 摘要；这些字段不能只写入 generation 的 `prompt_metadata`。
...
`builder-result.json` 保留结构化 candidate、校验和审计字段；其中 `files` 表示模型提交的原始候选文件内容。`skill-draft/` 目录是 `submit_candidate` 校验并处理引用后的最终 PSOP Skill Markdown 来源，必须把 `files` 中的 PSOP Skill 文件按相对路径物化为具体 Markdown 文件，例如 `README.md`、`SKILL.md`、`prompts/system.md`、`references/README.md`、`examples/input.md`、`examples/expected-output.md` 和 `tests/checklist.md`。如果物化阶段补齐参考图片 Markdown 链接，`builder-result.json.files` 与 `outputs/skill-draft/` 中的最终 Markdown 可以存在受控差异。物化文件只能由 `submit_candidate` 在校验通过后写入，模型不得通过 workspace tool 直接绕过校验写入 outputs。

如果候选产物选择了图片类参考资产，模型必须在使用该图片的流程步骤或对应说明中引用 `selected_reference_assets.reference_path`。`submit_candidate` 校验通过后负责读取平台准备的参考图片内容，把原图物化到 `outputs/skill-draft/` 下对应的 `references/` 相对路径，并在 `SKILL.md` 对应流程步骤中保留或补齐 Markdown 图片链接，例如 `![CPU 安装参考](references/video-keyframes/.../000950504.jpg)`；不得使用 `data:image/...;base64,...`，也不得把参考图片集中追加到 `SKILL.md` 或其他文档底部。
...
平台以最新一条 `status=succeeded`、Skill 相同、`committed_commit_sha` 与当前 source head 精确相等且 `raw_response.parsed` 可解析为 Candidate Schema v2 的 generation 作为 revision baseline。不存在历史成功记录时按首次全量生成；存在历史成功记录但当前 head 无精确基线时，视为源码被外部修改，preview 必须要求用户确认且确认前不得创建 generation/job。用户明确要求“全量重建/全部重写”时禁用继承。

`submit_candidate` 在普通校验前执行 baseline reconciliation。相同 ID 且完整业务对象一致的 safety constraint、相同 ID 且完整对象一致的 expected evidence，以及相同 ID、标题和对应 `SKILL.md` 阶段正文一致的 workflow 视为未变化。平台只为这些目标继承基线 `evidence_map` 和 `industry_standard_usage`；变化或新增目标必须由本轮用户指令、素材、参考资产或新标准结果覆盖。业务对象不变而只更换 ID 时返回 `stable_id_changed` 并给出原 ID。
...
`submit_candidate` 校验失败会完整收集并返回本次可判定的全部字段路径、错误码、允许值和最小修复示例，并携带 `diagnostic_count`；repair checklist、AgentEvent、middleware、timeline 和前端 API 不截断 diagnostics。失败 candidate 不物化、不提交 GitLab。首次提交后最多允许三次纠错提交（合计最多四次 candidate 提交，模型调用上限为 13）；同一完整诊断连续出现两次时提前以 `validation_failed` 终止。
...
```json
{
  ...
  "files": {
    "README.md": "# 零件清点\n\n用于在断电后完成零件清点。\n",
    "SKILL.md": "# 零件清点\n\n## Workflow\n### [stage_01_inventory] 零件清点\n确认设备断电，并对照参考图完成清点。\n![零件清点参考](references/video-keyframes/material-1/000001.jpg)\n",
    "prompts/system.md": "按 SKILL.md 的阶段、证据要求和安全约束推进。\n",
    "references/README.md": "# 参考资料\n\n![零件清点参考](video-keyframes/material-1/000001.jpg)\n",
  ...
  "evidence_map": [{
    ...
    "used_in": [
      {"target_type": "workflow_stage", "target_id": "stage_01_inventory"},
      {"target_type": "safety_constraint", "target_id": "safety_01_power_off"},
      {"target_type": "expected_evidence", "target_id": "evidence_01_inventory_photo"}
    ]
  }],
  "missing_questions": [],
  "safety_constraints": [{
    "constraint_id": "safety_01_power_off",
    "scope": "selected_stages",
    "stage_ids": ["stage_01_inventory"],
    "constraint": "清点前必须断电。",
    "risk_type": "electrical",
    "required_action": "未断电时停止操作。"
  }],
  "workflow_step_candidates": [{"stage_id": "stage_01_inventory", "title": "零件清点"}],
  "expected_evidence_requirements": [{
    "requirement_id": "evidence_01_inventory_photo",
    "stage_id": "stage_01_inventory",
    "evidence_type": "photo",
    "completion_criteria": "照片清晰显示零件和断电状态。"
  }]
}
```

上述示例的 `SKILL.md` 必须包含 `### [stage_01_inventory] 零件清点`，并在该阶段附近引用所选 `reference_path`。

候选结果的硬性 schema 要求：

| 字段 | 要求 | 最低校验 |
| --- | --- | --- |
| `schema_version` | 固定 Builder Candidate Schema 版本。 | 必填且严格等于 `"2.0"`。 |
...
| `safety_constraints` | 把安全要求绑定到阶段、动作、停止条件或恢复路径。 | 每项含唯一 `constraint_id`；`all_stages` 对应空 `stage_ids`，`selected_stages` 对应非空且合法的 `stage_ids`。 |
| `workflow_step_candidates` | 对应 SKILL.md 中阶段化 workflow 的候选结构。 | 每项含唯一 `stage_id`；`SKILL.md` 标题必须为 `### [stage_id] title`。 |
| `expected_evidence_requirements` | 描述每个关键阶段需要等待或收集的证据。 | 每项含唯一 `requirement_id`，并引用已声明 `stage_id`。 |
...
输入 schema 的唯一事实源是 `BuilderCandidate` Pydantic v2 模型，注册工具时使用 `BuilderCandidate.model_json_schema()` 的完整结果。LangChain 适配层必须以原始 JSON Schema 模式暴露该结果，保留 `$defs`、嵌套 `required`、枚举、pattern 和 `additionalProperties: false`，不得再把数组元素降级为 `items: {}`。关键结构如下：
...
```json
{
  ...
  "properties": {
    "schema_version": {"const": "2.0", "type": "string"},
    "directory_tree": {"type": "string", "minLength": 1},
    "files": {"type": "object"},
    "generation_reason": {"type": "string", "minLength": 1},
    "review_notes": {"type": "array", "items": {"type": "string"}},
    "material_usage": {"type": "array", "items": {"$ref": "#/$defs/MaterialUsageItem"}},
    "industry_standard_usage": {"type": "array", "items": {"$ref": "#/$defs/IndustryStandardUsageItem"}},
    "selected_reference_assets": {"type": "array", "items": {"$ref": "#/$defs/SelectedReferenceAsset"}},
    "evidence_map": {"type": "array", "items": {"$ref": "#/$defs/EvidenceMapItem"}},
    "missing_questions": {"type": "array", "items": {"$ref": "#/$defs/MissingQuestionItem"}},
    "safety_constraints": {"type": "array", "items": {"$ref": "#/$defs/SafetyConstraintItem"}},
    "workflow_step_candidates": {"type": "array", "items": {"$ref": "#/$defs/WorkflowStepCandidate"}},
    "expected_evidence_requirements": {"type": "array", "items": {"$ref": "#/$defs/ExpectedEvidenceRequirement"}}
  },
  "additionalProperties": false
}
```
...
- `safety_constraints.scope` 和 `stage_ids` 必须形成合法组合。
- 文件、素材、标准、参考资产、workflow、ID、交叉引用与 evidence coverage 校验都必须以 diagnostics collector 完整执行；单项检查不得抛出首个错误，最终统一去重和稳定排序后返回。
...
- `safety_constraints` 必须非空，每项含唯一 `constraint_id`；`scope=all_stages` 时 `stage_ids` 为空，`scope=selected_stages` 时 `stage_ids` 非空且只引用已声明阶段。
- `workflow_step_candidates` 与 `expected_evidence_requirements` 必须非空，分别含唯一 `stage_id` 与 `requirement_id`；`SKILL.md` 标题使用 `### [stage_id] title`。
- `evidence_map.used_in` 与 `industry_standard_usage.used_in` 必须使用结构化 `target_type`/`target_id`，且所有目标引用存在；`review_notes` 固定使用 `target_id=review_notes`。
- `selected_reference_assets` 必须来自候选资产，数量为 1 到 `MAX_SKILL_REFERENCE_ASSETS`，且 `stage_ids` 只引用已声明阶段。
...
- 上述 `reference_path` 校验适用于 builder candidate 和物化后的 PSOP Skill Markdown；最终 `SKILL.md` 必须在使用该图片的流程步骤附近通过相对 Markdown 图片链接显示该路径，图片文件由 `submit_candidate` 物化到 `outputs/skill-draft/references/`。
...
2. `submit_candidate` 校验 candidate，并将 PSOP Skill Markdown 物化到 sandbox；图片类参考资产在物化阶段保存为 `references/` 原图文件，并在使用位置保留或补齐相对 Markdown 图片链接。
