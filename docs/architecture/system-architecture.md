...
```text
backend/app/
  app.py                 FastAPI factory、lifespan、CORS、异常处理、OTel、事件监听
  worker_main.py         独立 runtime_job worker supervisor 入口
  main.py                uvicorn 入口
  api/
...
`psop-builder` 生成或更新 PSOP Skill draft。Builder 或人工可以把运行时参考图片提交到 Skill source 的 `references/` 目录，并在 `README.md` / `SKILL.md` 中用 Markdown 图片链接引用。`psop-compiler` 消费 PSOP Skill 并生成 PSOP-EG；发布编译时只接受 `references/` 下的相对图片路径，把图片镜像为受控 `ArtifactObject`，再写入 runtime contract。
...
download attachments / execute actor / call model or tools
lock job + Run and validate lease owner + snapshot version
merge observation
...
每次调用 timeout 不得超过剩余预算。外部 I/O 完成后的写事务必须同时校验 immutable
`JobLease` owner、Run snapshot 版本与单 Run job 归属；lease 丢失或版本变化时丢弃
observation，不提交部分 snapshot、trace success 或 terminal output。
...
的 PostgreSQL advisory lock，并用独立短 Session heartbeat。reaper 只回收 lease 已过期且
能够取得 advisory lock 的 job。API lifespan 默认不创建 worker；embedded 开关只用于
测试与紧急回滚。
...
PostgreSQL 部署中，worker commit 后通过 `psop_runtime_events` channel 发送只包含
`event_type/run_id/seq_no` 的 `NOTIFY` 提示。每个 API 副本 `LISTEN` 后用独立短 Session
加载完整事件并复用现有 WebSocket payload；SQLite 测试继续使用 in-process bus。
通知失败不回滚正式 Runtime 状态。

REST 读取 Run、TerminalEvent、TraceEvent、Snapshot 和 Replay 仍是断线恢复与权威补齐路径。

低基数 OTel metrics 覆盖 terminal upload/object bytes 与 duration、content
200/206/304/416 和 S3 实读字节、Object Store executor queue/in-flight、event-loop
lag、DB checkout wait，以及 job claim/duration/retry/lease recovery/step timeout。`run_id`、
`job_id`、owner 与 object key 只进入日志/span，不进入 metric labels。
