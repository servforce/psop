...
代码位置：`backend/app/domain/skills/*`
...
- 发布 Skill 时冻结 GitLab branch head，创建 published `SkillVersion`、`SkillPublishRecord`、`SkillCompileRequest` 和 `RuntimeJob(job_type=compile)`。
- 管理 raw materials：上传、对象存储、分析、派生资产、从视频素材生成 Skill draft。

当前 Skill source 事实源：

- 用户可读源码：GitLab repository。
- 结构化机器契约：`skill_version.manifest_snapshot`。
- `skill.yaml` 当前由服务端生成并写回 GitLab，作为结构化契约的序列化视图。

### 7.2 Compiler

代码位置：`backend/app/domain/compiler/*`
...
2. 根据 terminal cursor 补读 `terminal_event`，把新 input 绑定到当前 wait checkpoint。
...
8. 追加 snapshot 与 trace event。
9. 命中 success/aborted/wait/failed 条件后更新 run 状态。

当前能力调用边界：

- LLM 由 `LlmInferenceGateway` 执行。
- 多模态 LLM 由 terminal event part 中的对象存储内容转 base64 后进入 OpenAI-compatible content。
- tool 只有内置 demo tool。
- 当前没有独立 `CapabilityHost` 类，能力解析和调用集中在 `RuntimeService`。

### 7.4 Terminal Gateway

代码位置：`backend/app/api/routes/runtime.py` 和 `RuntimeService`

职责：

- `POST /terminal/sessions/{run_id}/events` 支持 JSON text input 和 multipart multimodal input。
...
- 文件写入对象存储，同时创建 `artifact_object` 和 `terminal_event_part.artifact_object_id`。
- `Idempotency-Key` 或 `external_event_id` 映射到 `run_id + external_event_id` 去重。
- 事件 append 后通过当前进程内 WebSocket hub 广播新 terminal events；REST 仍是权威状态源。

### 7.5 Jobs

代码位置：`backend/app/domain/jobs/*`
...
- 常用状态：`pending`、`running`、`succeeded`、`failed`、`cancelled`。
- read model 兼容：`retryable_failed`、`canceled`、`deadletter`、`dead_letter`。
...
- 非 compile job 的未处理异常按 `attempt_no < max_attempts` 退回 `pending` 或置 `failed`。
- 当前没有独立 scheduler 恢复过期 lease。

### 7.6 Skill Tests

代码位置：`backend/app/domain/skill_tests/*`

职责：

- 管理 `SkillTestScenario`、`SkillTestAsset`、`SkillTestScenarioRun`。
- 启动测试时创建真实 invocation/run/terminal session。
- 写入 `RuntimeJob(job_type=skill_test_timeline_driver)`。
...
代码位置：`backend/app/domain/agent_prompts/*`、`backend/app/agents/*`
...
| 表 | 关键字段 | 说明 |
| --- | --- | --- |
| `skill_definition` | `key`、`name`、`status`、`gitlab_project_id`、`repository_url`、`default_branch`、`manifest_path`、`latest_draft_version_id`、`latest_published_version_id` | Skill 总对象和 GitLab 绑定 |
| `skill_version` | `skill_definition_id`、`version_no`、`status`、`source_ref`、`source_commit_sha`、`manifest_snapshot`、`runtime_policy_snapshot` | draft/published 版本 |
| `skill_publish_record` | `skill_definition_id`、`skill_version_id`、`publish_reason`、`publish_status`、`published_commit_sha`、`release_ref` | 发布记录 |
| `skill_raw_material` | `skill_definition_id`、`artifact_object_id`、`material_kind`、`mime_type`、`status`、`size_bytes`、`checksum` | Skill 素材 |
| `skill_raw_material_generation` | `skill_definition_id`、`material_ids`、`status`、`prompt_metadata`、`generated_files`、`committed_commit_sha` | 从素材生成 Skill draft |
| `skill_raw_material_analysis` | `skill_definition_id`、`raw_material_id`、`status`、`analysis_result`、`error_details` | 素材分析 |
| `skill_raw_material_derived_asset` | `raw_material_id`、`analysis_id`、`artifact_object_id`、`asset_kind`、`timestamp_ms` | 视频关键帧等派生资产 |

### 8.2 Compiler / Artifact

| 表 | 关键字段 | 说明 |
| --- | --- | --- |
| `skill_compile_request` | `skill_definition_id`、`skill_version_id`、`trigger_type`、`source_commit_sha`、`status`、`dedupe_key` | 编译请求 |
| `artifact_object` | `bucket`、`object_key`、`media_type`、`size_bytes`、`checksum`、`content_json` | 统一对象索引；JSON artifact 当前直接保存在 `content_json` |
| `eg_compile_artifact` | `skill_compile_request_id`、`skill_version_id`、`artifact_object_id`、`formal_revision`、`artifact_version`、`graph_summary`、`capability_summary`、`status` | Runtime 正式执行输入索引 |
| `compile_diagnostic` | `skill_compile_request_id`、`skill_version_id`、`severity`、`code`、`message`、`location`、`category` | 编译诊断 |

### 8.3 Runtime / Terminal / Replay

| 表 | 关键字段 | 说明 |
| --- | --- | --- |
| `skill_invocation` | `skill_definition_id`、`skill_version_id`、`compile_artifact_id`、`gateway_type`、`input_envelope`、`terminal_context`、`binding_preferences`、`status`、`idempotency_key` | Gateway 调用对象 |
| `run` | `invocation_id`、`skill_definition_id`、`skill_version_id`、`compile_artifact_id`、`terminal_session_id`、`status`、`runtime_phase`、`latest_*_seq`、`final_output`、`exit_reason` | 逻辑运行实例 |
| `session_token_snapshot` | `run_id`、`seq_no`、`token_payload`、`enabled_set`、`selection_summary`、`snapshot_hash` | 正式状态快照 |
| `trace_event` | `run_id`、`seq_no`、`phase`、`event_type`、`span_id`、`payload` | replay/observability 事件 |
...
| `terminal_event` | `terminal_session_id`、`run_id`、`trace_event_id`、`artifact_object_id`、`run_capability_binding_id`、`direction`、`event_kind`、`payload_inline`、`seq_no`、`external_event_id` | append-only transcript |
| `terminal_event_part` | `terminal_event_id`、`run_id`、`artifact_object_id`、`part_id`、`order_index`、`kind`、`mime_type`、`text_inline`、`checksum`、`metadata` | 多模态输入 part |
...
| `skill_test_scenario` | `skill_definition_id`、`target_compile_artifact_id`、`duration_ms`、`timeline`、`judge_policy`、`fork_seed`、`status` | 测试场景 |
| `skill_test_asset` | `skill_definition_id`、`scenario_id`、`artifact_object_id`、`lane_id`、`filename`、`mime_type`、`checksum` | 场景资源 |
| `skill_test_scenario_run` | `skill_definition_id`、`scenario_id`、`invocation_id`、`run_id`、`status`、`driver_status`、`driver_cursor`、`driver_events`、`timeline`、`result_summary` | 一次测试运行 |
...
| `POST` | `/api/v1/skills/{skill_id}/raw-materials` | 上传素材 |
| `GET` | `/api/v1/skills/{skill_id}/raw-materials` | 素材列表 |
| `POST` | `/api/v1/skills/{skill_id}/raw-materials/generate-skill-draft` | 从素材生成 Skill draft |
| `POST` | `/api/v1/skills/{skill_id}/raw-materials/{material_id}/analyze` | 重新分析素材 |
| `GET` | `/api/v1/skills/{skill_id}/raw-materials/{material_id}/analysis` | 读取分析 |
| `GET` | `/api/v1/skills/{skill_id}/raw-materials/{material_id}` | 素材详情 |
| `GET` | `/api/v1/skills/{skill_id}/raw-materials/{material_id}/content` | 素材内容，支持 Range |
| `GET` | `/api/v1/skills/{skill_id}/raw-materials/{material_id}/derived-assets/{asset_id}/content` | 派生资产内容 |
| `DELETE` | `/api/v1/skills/{skill_id}/raw-materials/{material_id}` | 归档素材 |
...
| `GET` | `/api/v1/runs/{run_id}/trace-events` | trace events；支持 `event_type` |
...
| `GET` | `/api/v1/terminal/sessions/{run_id}/events` | terminal events；支持 `from_seq`、`to_seq` |
| `POST` | `/api/v1/terminal/sessions/{run_id}/events` | 追加 JSON/multipart terminal event |
| `GET` | `/api/v1/terminal/sessions/{run_id}/events/{event_id}/content` | 事件级对象内容，支持 Range |
| `GET` | `/api/v1/terminal/sessions/{run_id}/events/{event_id}/parts/{part_id}/content` | part 内容，支持 Range |
...
- REST 成功追加 terminal event 后，当前进程内 hub 广播 `terminal.event.appended`。
...
运行时 trace 事件写入数据库 `trace_event`，Replay 直接读取持久化事件，不推断未落库状态。
...
- 无独立 scheduler；过期 lease 自动恢复和 dead-letter 策略未完整落地。
