...
- claim 前恢复过期 lease：`running and lease_until <= now` 的任务若 `attempt_no < max_attempts`，回到 `pending` 并按 `5 * attempt_no` 秒退避；否则置为 `failed`。
...
- 非 `pskill_compile` job（含旧 `compile` 兼容类型）的未处理异常按 `attempt_no < max_attempts` 退回 `pending` 或置 `failed`。
...
- 无独立 scheduler；过期 lease 已由内置 worker 恢复，独立 scheduler / dead-letter 策略未完整落地。
