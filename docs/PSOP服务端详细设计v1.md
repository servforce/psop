...
- 编译关键 span（`job.compile`、`compile.source_load`、`compile.agent.invoke`、`compile.validate`、`compile.emit`）携带 `compile_request_id`、`pskill_definition_id`、`pskill_version_id` 和成功后的 `compile_artifact_id`，用于 Publish / Auto Compile 与 OTel 排障关联。
- `CompileRequestResponse` 内嵌轻量 `progress` 摘要，包含当前阶段、阶段状态、完成百分比和 terminal 状态，供 Compiler Workbench 列表直接展示。
- `CompileArtifactResponse` 内嵌对应 `compile_request` 摘要，Artifact 详情可以直接回跳编译请求、`pskill.compiler` AgentRun 和 progress 证据。
...
- 构建 Replay detail。
- `InvocationResponse` 与 `RunResponse` 派生返回 `compile_request_id`，从 `compile_artifact_id` 反查，不在 Runtime 正式状态表重复写入。
...
- `POST /runs/{run_id}/events` 支持 JSON text input 和 multipart multimodal input；旧 `/terminal/sessions/{run_id}/events` 仅作为短期兼容入口，不再作为新代码调用路径。
...
| `GET` | `/api/v1/compiler/artifacts/{compile_artifact_id}` | artifact 详情，含 `artifact` JSON 与 `compile_request` 摘要 |
...
| `GET` | `/api/v1/runs/{run_id}/terminal-session` | terminal session 摘要 |
