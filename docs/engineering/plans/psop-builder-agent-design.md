...
Harness / 应用代码负责：
  ...
  - 解析 selected_reference_assets，复制参考资产。
...
workspace 工具输出必须包含 `virtual_path`、`bytes`、`truncated` 和 `artifact_ref`。`workspace.write_text` 不得写入 `/mnt/psop/outputs/builder-result.json`，避免绕过 `submit_candidate` 校验。

### 7. Candidate 提交工具

`psop.builder.submit_candidate` 是 builder 唯一允许写入最终候选产物的工具。它不是发布工具，不提交 GitLab，不更新数据库业务状态，只在 sandbox output 目录生成可由 `SkillsService` 读取和二次校验的 artifact。
...
```text
1. agent 只写 builder-result artifact。
2. SkillsService 读取并校验 artifact。
3. SkillsService 再次检查 GitLab branch head。
4. SkillsService 复用 _resolve_selected_reference_assets 复制参考资产。
5. SkillsService 复用 _commit_generated_skill_files 提交 README/SKILL/辅助文件和由平台渲染的 skill.yaml。
6. draft_version.source_commit_sha、manifest_snapshot、runtime_policy_snapshot 由应用层更新。
```
