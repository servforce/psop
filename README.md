...
运行时，一次真实 invocation 会创建 Run、Terminal Session、run traces、run events 与 Session Token snapshots。用户反馈、图片、音频、视频、文件、设备确认和现场观察都会作为终端事实进入系统。Replay 与可观测性是默认排障闭环，用于复盘任务执行过程，并为后续 Skill 迭代提供依据。

平台围绕以下核心约束设计：

- `Session Token` 是唯一正式运行时状态对象。
- `Runtime Kernel` 是唯一正式状态主权者。
- `run_event` 是终端输入输出的 append-only 事实源。
...
```text
backend/      FastAPI 服务端、编译、运行时、任务、Gateway 与 Repository
static/       基于 Alpine.js 与 Tailwind CSS 的静态 Web IDE
docs/         产品纲领、形式定义、概要设计、前端设计与服务端设计文档
...
- `backend/app/pskills/`：PSkill 元数据、源码、发布与 GitLab 集成。
- `backend/app/compiler/`：formal-v5 编译请求、诊断与 EG 产物。
- `backend/app/runtime/`：invocation、run、Session Token snapshot、run event、run trace 与 replay。
- `backend/app/testing/`：黑盒时序测试场景、资源、运行与 Judge 评估。
...
- [PSOP Whitepaper](docs/PSOP-Whitepaper-v3.md)
- [Execution Graph 形式定义](docs/PSOP_execution_graph_formal_v5.md)
- [系统概要设计](docs/PSOP概要设计v1.md)
- [前端详细设计](docs/PSOP前端详细设计v1.md)
- [服务端详细设计](docs/PSOP服务端详细设计v1.md)
...
当前仓库主要通过 feature branch 开发。实现变更需要与 `docs/` 根目录下的设计基线保持一致；如果行为发生变化，应先更新或同步更新对应设计文档，再推进代码实现。
