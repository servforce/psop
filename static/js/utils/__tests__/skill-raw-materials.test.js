...
test("skill detail exposes raw materials tab and generation workflow", () => {
  ...
  expect(html).toContain("builderAgentPanel.open");
  expect(html).toContain("builderAgentPanel.result.committed_commit_sha");
  expect(html).toContain("builderAgentFinalReferenceFiles()");
  expect(html).toContain("builderAgentWorkLabel()");
  expect(html).toContain("builderAgentProgressPercent()");
  expect(html).not.toContain('class="modal-panel flex max-h-[92vh] max-w-3xl flex-col"');
  ...
  expect(skillDetailJs).toContain('submittedInput: userDescription');
  expect(skillDetailJs).toContain('userInput: ""');
  expect(skillDetailJs).not.toContain("userInput: userDescription");
  expect(skillDetailJs).not.toContain("this.builderAgentPanel.userInput || submittedInput");
  ...
  expect(skillDetailJs).toContain("/agents/runs/latest");
  expect(skillDetailJs).toContain("/timeline");
  expect(skillDetailJs).toContain("new EventSource");
  expect(skillDetailJs).toContain("startBuilderAgentPolling");
  expect(skillDetailJs).not.toContain('await this.navigate("/admin/tasks")');
  expect(skillDetailJs).not.toContain('this.showCenterToast("success", "Skill 生成任务已提交。")');
  expect(skillDetailJs).not.toContain('jobId ? `Skill 生成任务已提交：${this.formatShortId(jobId)}`');
