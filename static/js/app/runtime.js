(function () {
  ...
  window.PSOPConsoleRuntimeMethods = {
      ...
      liveRunToolAuthorizationContextLinks(authorization) {
        const links = [];
        const agentRunId = String(authorization?.agent_run_id || "").trim();
        const toolCallId = String(authorization?.agent_tool_call_id || "").trim();
        const runId = String(authorization?.run_id || this.liveRunToolAuthorizationFirstNestedValue(authorization, [
          "run_id",
          "source_run_id"
        ]) || this.liveRun?.id || "").trim();
        const runEventId = String(authorization?.run_event_id || this.liveRunToolAuthorizationFirstNestedValue(authorization, [
          "run_event_id",
          "event_id"
        ]) || "").trim();
        const proposalId = this.liveRunToolAuthorizationFirstNestedValue(authorization, [
          "proposal_id",
          "governance_proposal_id"
        ]);
        const experimentId = this.liveRunToolAuthorizationFirstNestedValue(authorization, [
          "experiment_id",
          "governance_experiment_id"
        ]);
        const evaluationId = this.liveRunToolAuthorizationFirstNestedValue(authorization, [
          "evaluation_id",
          "evaluation_report_id",
          "run_evaluation_id",
          "source_evaluation_id"
        ]);
        const findingIds = this.liveRunToolAuthorizationNestedValues(authorization, [
          "finding_id",
        ...
        const pskillId = this.liveRunToolAuthorizationFirstNestedValue(authorization, [
        ...
        if (agentRunId) {
          links.push({
            ...
            label: "AgentRun",
            value: agentRunId,
            href: this.liveRunReplayAgentRunPath(agentRunId, { tab: "authorizations", authorization_id: authorization?.id || "" }),
            icon: "smart_toy"
          });
        }
        if (agentRunId && toolCallId) {
          links.push({
            ...
            label: "ToolCall",
            value: toolCallId,
            href: this.liveRunReplayAgentRunPath(agentRunId, { tab: "tools", tool_call_id: toolCallId }),
            icon: "build"
          });
        }
        if (runId) {
        ...
        return this.uniqueLiveRunToolAuthorizationLinks(links);
      },


      uniqueLiveRunToolAuthorizationLinks(links) {
        const seen = new Set();
      ...
      liveRunToolAuthorizationFirstNestedValue(authorization, keys) {
        return this.liveRunToolAuthorizationNestedValues(authorization, keys)[0] || "";
      },


      liveRunToolAuthorizationNestedValues(authorization, keys) {
