(function () {
  ...
  window.PSOPConsolePlatformMethods = {
    ...
    agentRunAuthorizationContextLinks(authorization) {
      const links = [];
      const agentRunId = String(authorization?.agent_run_id || this.currentAgentRun?.id || "").trim();
      const toolCallId = String(authorization?.agent_tool_call_id || "").trim();
      const runId = String(authorization?.run_id || this.agentRunAuthorizationFirstNestedValue(authorization, [
        "run_id",
        "source_run_id"
      ]) || this.currentAgentRun?.run_id || "").trim();
      const runEventId = String(authorization?.run_event_id || this.agentRunAuthorizationFirstNestedValue(authorization, [
        "run_event_id",
        "event_id"
      ]) || "").trim();
      const proposalId = this.agentRunAuthorizationFirstNestedValue(authorization, [
        "proposal_id",
        "governance_proposal_id"
      ]);
      const experimentId = this.agentRunAuthorizationFirstNestedValue(authorization, [
        "experiment_id",
        "governance_experiment_id"
      ]);
      const evaluationId = this.agentRunAuthorizationFirstNestedValue(authorization, [
        "evaluation_id",
        "evaluation_report_id",
        "run_evaluation_id",
        "source_evaluation_id"
      ]);
      const findingIds = this.agentRunAuthorizationNestedValues(authorization, [
        "finding_id",
      ...
      const pskillId = this.agentRunAuthorizationFirstNestedValue(authorization, [
      ...
      return this.uniqueAgentRunAuthorizationLinks(links);
    },

    uniqueAgentRunAuthorizationLinks(links) {
      const seen = new Set();
    ...
    agentRunAuthorizationFirstNestedValue(authorization, keys) {
      return this.agentRunAuthorizationNestedValues(authorization, keys)[0] || "";
    },

    agentRunAuthorizationNestedValues(authorization, keys) {
