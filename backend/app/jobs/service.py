...
from app.jobs.models import RuntimeJob, TERMINAL_JOB_STATUSES
...
class JobQueryService:
    ...
    def _material_analysis_progress(self, session: Session, job: RuntimeJob) -> RuntimeJobProgressResponse:
        analysis_id = str((job.payload or {}).get("analysis_id") or "")
        analysis = session.get(PSkillMaterialAnalysis, analysis_id) if analysis_id else None
        if not analysis:
            return self._fallback_progress(job)
        percent_by_status = {
            "pending": 0,
    ...
    def _skill_generation_progress(self, session: Session, job: RuntimeJob) -> RuntimeJobProgressResponse:
        ...
        generation = session.get(PSkillMaterialGeneration, generation_id) if generation_id else None
