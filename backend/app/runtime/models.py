...
class Run(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_run_pskill_definition_created_at", "pskill_definition_id", "created_at"),
    )
    ...
    latest_run_event_seq: Mapped[int] = mapped_column(default=0, nullable=False)
...
class RunEvent(Base):
    ...
    __table_args__ = (
        UniqueConstraint("terminal_session_id", "seq_no", name="uk_run_event_session_seq"),
...
class RunEventPart(Base):
    __tablename__ = "run_event_part"
    __table_args__ = (
        UniqueConstraint("run_event_id", "part_id", name="uk_run_event_part_event_part"),
        UniqueConstraint("run_event_id", "order_index", name="uk_run_event_part_event_order"),
        Index("idx_run_event_part_event_order", "run_event_id", "order_index"),
        Index("idx_run_event_part_run", "run_id", "order_index"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    run_event_id: Mapped[str] = mapped_column(
        ForeignKey("run_event.id", ondelete="CASCADE"),
        nullable=False,
    )
    run_id: Mapped[str] = mapped_column(ForeignKey("run.id", ondelete="CASCADE"), nullable=False)
    artifact_object_id: Mapped[str | None] = mapped_column(
