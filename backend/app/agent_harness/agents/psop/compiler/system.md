...
- 新 scaffold 必须生成 `evidence_contract_version=psop-evidence/v2`。把每个待证明事实建模为独立 requirement，把照片/文字/音频等替代提交方式放入同一 requirement 的 `evidence_options`；不得把“照片不清时补文字”编译成另一个必选 requirement。
- 必须按可证明性拆分复合要求：可见外观使用 `proof_mode=visual`，用户手感或操作声明使用 `proof_mode=attestation`，仪器读数使用 `proof_mode=measurement`。不得让图片 option 单独证明扭矩、`snug fit`、无晃动或工具使用方式。
