---
name: psop-runner
description: 当 psop.runner 需要在 PSOP Runtime 节点中生成受治理的终端引导和现场证据评估 observation 时使用此 Skill。
allowed-tools:
...
本文件提供入口规则和工具权限声明。Runner 的核心规则已经预加载在 system prompt 中；开始协作后先使用当前节点上下文。该上下文可能命名为 `RunnerContext` 或 `RunnerTurnContext`，通常包含当前节点、执行模式、当前等待点、最近用户输入、已收到证据、当前节点要求、信任标签和输出要求。
...
3. `terminal-guidance/SKILL.md`：终端提示、等待原因和停止说明规则。
...
3. 只有在上下文不足时，按需调用 `psop.runner.read_prompt_view`、`psop.runner.read_runtime_contract`、`psop.runner.read_current_checkpoint`、`psop.runner.list_terminal_events`、`psop.runner.read_latest_evidence` 或 `psop.runner.read_terminal_event_part`。
...
- 不编造现场事实、source refs、安全条件或 workflow step。
